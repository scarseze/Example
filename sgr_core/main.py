...
async def main():
    ...
    configure_logger()
    logger.info("SGR Core Agent Initializing")
    ...
    if proxy_url:
        ...
        llm_config = {
            "base_url": proxy_url + "/v1", # Proxy appends /v1
            "api_key": "dummy-key",        # Key is injected by proxy
            "model": "deepseek-chat"
        }
    elif deepseek_key:
    ...
    # engine.register_skill(XBRLAnalystSkill()) # Disabled to prevent PDF routing confusion
    ...
    # Example MCP Skill Registration (Uncomment to use)
    # from skills.mcp_adapter.handler import McpSkill
    # engine.register_skill(McpSkill(
    #    server_name="filesystem",
    #    command="docker",
    #    args=["run", "-i", "--rm", "-v", "./data:/data", "mcp/filesystem", "/data"]
    # ))
...
if __name__ == "__main__":
    ...
    if args.max:
        ...
        # Setup Engine (Duplicated config logic for now, should be refactored)
        configure_logger()
        logger.info("Initializing SGR Core (Max Mode)...")
        
        # Load Env
        # (Same Loading Logic)
        
        # Init Engine
        engine = CoreEngine() # Will load config internally or via defaults
        # Register Skills
        engine.register_skill(SGLangSkill())
        engine.register_skill(PortfolioSkill())
        engine.register_skill(GostWriterSkill())
